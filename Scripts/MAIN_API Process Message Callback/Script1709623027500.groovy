...
for (GlobalVariable.NumofColm = 2; GlobalVariable.NumofColm <= countColmExcel; (GlobalVariable.NumofColm)++) {
    if (findTestData(excelPath).getValue(GlobalVariable.NumofColm, rowExcel('Status')).length() == 0) {
        ...
        if (GlobalVariable.AdInsKey == '') {
            ...
            'HIT API'
            respon = WS.sendRequest(findTestObject('Postman/Process Message Callback', [('requestDateTime') : findTestData(excelPath).getValue(
                            GlobalVariable.NumofColm, rowExcel('requestDateTime')), ('rowVersion') : findTestData(excelPath).getValue(
                            GlobalVariable.NumofColm, rowExcel('rowVersion')), ('receiverPhone') : findTestData(excelPath).getValue(
                            ...
                            GlobalVariable.NumofColm, rowExcel('buttonParam')), ('requestUuid') : findTestData(excelPath).getValue(
                            GlobalVariable.NumofColm, rowExcel('requestUuid')), ('partnerCode') : findTestData(excelPath).getValue(
